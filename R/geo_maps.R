...
geo_map <-  function (world, d2, output = NA){
  ...
  us_variation_map
  if (!is.na(output))
    ggsave(filename =  paste(output, '/us_variation.png', sep =""),
  ...
  if (!is.na(output))
    ggsave(filename = paste(output, '/world_variation_country.png', sep =""),
  ...
  if (!is.na(output))
    ggsave(filename = paste(output,',fig5_location_variation.png', sep =""),
