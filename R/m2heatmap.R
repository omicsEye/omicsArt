...
m2heatmap <- function(O, P, Ocil, Ociu, title, stars = F) {
  ...
  ggp <- ggplot(data = df, aes(x = V1, y = V2)) +
    ...
    scale_fill_gradientn(
      colours = colorRampPalette(brewer.pal(n = 9, name = "Blues"))(100),
      values = colorvalues,
  ...
  if (!missing(P)) {
    ggp <- ggp + if (stars) {
      ...
      geom_text(aes(
        label = ifelse(is.na(padj), "", sprintf("%.2g", padj)),
        color = lblcolor
      ), size = 1.7)
    }
  }
  ggp <- ggp + if (!missing(Ocil)) {
    geom_text(
      aes(
        label = ifelse(is.na(obs), "", sprintf("%.1f%%\n[%.1f%% - %.1f%%]", 100 * obs, 100 * cil, 100 * ciu)),
        color = lblcolor
      ),
    ...
    geom_text(
      aes(
        label = ifelse(is.na(obs), "", sprintf("%.1f%%", 100 * obs)),
        color = lblcolor
      ),
      size = 1.7, nudge_y = nudge_y
    )
