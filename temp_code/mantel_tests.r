...
for (i in seq_along(pcls)) {
    for (j in seq_along(pcls)) {
        if (i > j && is.na(mt_inter_doa$C[i,j])) {
            ...
            mt_intra$Cil[i,j] <- quantile(mt$bootstraps, 0.025)
            mt_intra$Ciu[i,j] <- quantile(mt$bootstraps, 0.975)
            ...
            mt_inter_doa$Cil[i,j] <- quantile(mt$bootstraps, 0.025)
            mt_inter_doa$Ciu[i,j] <- quantile(mt$bootstraps, 0.975)
...
save(mt_intra, file=sprintf("./overview/mantel_test_intra.RData"))
#save(mt_inter_aod, file=sprintf("./overview/mantel_test_inter_aod.RData"))
save(mt_inter_doa, file=sprintf("./overview/mantel_test_inter_doa.RData"))
...
manteltest_plot <- function(O, P, Ocil, Ociu, title, stars=F) {
    ...
    df$obs <- df$obs^2
    df$lblcolor <- ifelse(is.na(df$obs) | (df$obs < 0.7), "black", "white")
    if (!missing(P)) {
        Padj <- matrix(p.adjust(P, method="fdr"), nrow=nrow(P))
        df$padj <- melt(Padj)$value
    }
    if (!missing(Ocil)) {
        df$cil <- melt(Ocil)$value
        df$ciu <- melt(Ociu)$value
    }
    ...
    ggp <- ggplot(data=df, aes(x=V1, y=V2)) +
        ...
        scale_fill_gradientn(colours=colorRampPalette(brewer.pal(n = 9, name = "Blues"))(100),
                             na.value="white", limits=c(0, 1), name="Correlation")
    nudge_y <- 0
    if (!missing(P)) {
        ggp <- ggp + if (stars) {
            nudge_y <- -0.14
    ...
    ggp <- ggp + if (!missing(Ocil)) {
        geom_text(aes(label=ifelse(is.na(obs), "", sprintf("%.3f\n[%.3f - %.3f]", obs, cil, ciu)),
                      color=lblcolor),
                  size=1.7, nudge_y=nudge_y)
    } else {
        geom_text(aes(label=ifelse(is.na(obs), "", sprintf("%.3f", obs)),
                      color=lblcolor),
                  size=1.7, nudge_y=nudge_y)
    }
    ggp <- ggp +
...
pdf("./overview/mantel_tests_fig1.pdf", 2.9, 1.4)
